...
function readTextFile(file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => {
      reject(new Error(`读取文件“${file.name}”失败，请重试。`));
    };
...
async function handleTxtFilesChange(event) {
  ...
  if (invalidFiles.length > 0) {
    error.value = `仅支持 .txt 文件，以下文件不符合要求：${invalidFiles
      .map((file) => file.name)
      .join("、")}`;
    input.value = "";
    return;
  ...
  try {
    const lines = await Promise.all(
      files.map(async (file) => {
        ...
        if (!content.trim()) {
          throw new Error(`文件“${file.name}”内容为空，请检查后重试。`);
        }

        const title = file.name.replace(/\.txt$/i, "").trim() || "未命名文档";
        ...
        return `${title}::${singleLineContent}`;
      }),
    );
...
async function handleCluster() {
  ...
  try {
    ...
    error.value =
      e instanceof Error ? e.message : "输入格式有误，请检查后重试。";
    hasSubmitted.value = false;
    return;
  }
    ...
    <section class="workspace-card">
      <div class="section-head">
        <div>
          <h3>输入文档</h3>
          <p>
            每行一条文档，格式为“标题::内容”。至少输入两条文档以生成聚类结果。
          </p>
        </div>
      </div>

      <div class="upload-panel">
        <p class="upload-note">每个 TXT 文件对应一篇文章，文件名作为标题。</p>
      ...
      <div class="form-grid">
        ...
        <div class="field-note">
          <strong>输入提示</strong>
          <p>
            建议准备 3 至 6 条主题差异明显的文本，更容易得到适合截图的聚类分布。
          </p>
        </div>
      </div>
      ...
      <div class="action-row">
        ...
        <button class="ghost-btn" :disabled="loading" @click="clearAll">
          清空内容
        </button>
      </div>
    </section>
    ...
    <section class="workspace-card">
      <div class="section-head">
        <div>
          <h3>结果说明</h3>
          <p>
            图表用于展示文档相对位置，右侧摘要用于说明每个簇中包含哪些文档。
          </p>
        </div>
      </div>

      <div v-if="error" class="status-banner status-error" role="alert">
        {{ error }}
      </div>
      ...
      <div
        v-else-if="hasSubmitted && !hasResults"
        class="status-banner status-empty"
      >
