...
- `POST /api/classify`
- `POST /api/cluster`
- `GET /api/meta`（前端单文本页面用于展示后端在线状态、服务名称与 NER 运行路径）
...
- NER 标签采用受控、可读的类别集合：`PER`、`LOC`、`ORG`、`MISC`、`GPE`、`FAC`、`COMPANY`、`INSTITUTION`
- 为保持课程项目可解释性，未纳入受控集合的标签会收敛到 `MISC`，避免输出过多零散类别
...
### 11.1 NER 真实环境手动验收（本地）

用于课程演示阶段快速确认真实后端 + 真实 HTTP 请求下的 NER 结果是否符合预期，不作为 CI 依赖。
...
```bash
python scripts/validate_ner_examples.py
```

如需改为其他后端地址：

```bash
python scripts/validate_ner_examples.py --base-url http://127.0.0.1:5001
```

推荐验收句子与高层期望：

- 马云在杭州创办了阿里巴巴集团。期望至少识别出 `PER`、`LOC`、`ORG/COMPANY`。
- 腾讯公司在深圳发布了新产品。期望至少识别出 `ORG/COMPANY`、`LOC`。
- 张三在上海交通大学参加学术会议。期望至少识别出 `PER`、`ORG/INSTITUTION`。
- 北京市政府与北京大学合作。期望至少识别出 `INSTITUTION`（或稳定映射到受控机构类）。

结果解读建议：

- 先看是否返回 `success=true` 且实体结构为 `text/label/start/end`。
- 再看标签是否落在受控集合（如 `PER`、`LOC`、`ORG`、`MISC`、`GPE`、`FAC`、`COMPANY`、`INSTITUTION`）。
- 最后按“高层类别”对比是否基本符合预期；允许模型细节存在轻微波动。
